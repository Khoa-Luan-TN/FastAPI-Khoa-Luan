...
const ROOT_SECTIONS = [
  {
    ...
    name: "documents",
    label: "Tài liệu",
    desc: "Sách giáo khoa, bài học, chunk",
    bg: "#EFF6FF",
    color: "#2563EB",
    Icon: FolderIcon,
  },
  ...
  const fileRows = useMemo(() => {
    ...
    const list = (remote.files || []).map((x) => ({
      ...
      updatedAt: x.last_modified
        ? (() => {
            const d = x.last_modified.slice(0, 10).split("-");
            return d.length === 3 ? `${d[2]}/${d[1]}/${d[0]}` : x.last_modified.slice(0, 10);
          })()
        : "",
        ...
        >
          ...
          <div className="minio-action-bar">
            <div className="minio-search">
              <span className="minio-search-icon">
                <SearchIcon />
              </span>
              <input
        ...
        <div className="minio-root-grid">
          {ROOT_SECTIONS.map((s) => (
            <div key={s.id} className="minio-root-card" onClick={() => navigateTo(s.name)}>
              <div
                className="mrc-banner"
                style={{
                  background: `linear-gradient(135deg, ${s.color}18 0%, ${s.color}38 100%)`,
                }}
              >
          ...
          <div className="minio-file-list">
            <div className="minio-file-list-inner">
              ...
              {fileRows.map((row) => {
                ...
                const TypeIcon =
                  type === "video" ? VideoIcon : type === "image" ? ImageIcon : FileIcon;
                return (
                  <div
                    key={row.id}
                  ...
                  >
                    ...
                    <div className="mfi-actions-cell">
                      <button className="mfi-action-btn" onClick={(e) => editFile(row, e)}>
                        <EditIcon /> Sửa
                      </button>
                      <button className="mfi-action-btn danger" onClick={(e) => deleteFile(row, e)}>
                        <TrashIcon /> Xoá
                      </button>
                    </div>
                  </div>
