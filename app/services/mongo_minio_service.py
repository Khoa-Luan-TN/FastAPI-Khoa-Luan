...
def on_minio_insert_to_mongo(
    *,
    ...
    """File uploaded to MinIO → create or update asset record."""
    ...
    if existing:
        ...
        return {"ok": True, "mode": "update", "collection": "asset", "_id": str(existing["_id"])}
    ...
    result = db["asset"].insert_one(doc)
    return {"ok": True, "mode": "create", "collection": "asset", "_id": str(result.inserted_id)}
